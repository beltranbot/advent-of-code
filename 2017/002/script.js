...
function checkSum (data) {
  ...
  for (const row in data) {
    var largest = data[row][0]
    var smallest = data[row][0]

    for (const i in data[row]) {
      const number = data[row][i]
      if (number > largest) {
        largest = number
...
function part2 (data) {
  ...
  for (const row in data) {
    var orderedRow = data[row].sort((a, b) => b - a)
    for (var i = 0; i < orderedRow.length; i++) {
      for (var j = 0; j < orderedRow.length; j++) {
        var num = orderedRow[i]
        var div = orderedRow[j]
        if ((i !== j) && (num % div === 0)) {
          result += (num / div)
          break
        }
